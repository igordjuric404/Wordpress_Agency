...
const Hoverable = forwardRef<HTMLElement, HoverableProps>(
  ({ as: Component = 'div', children, disabled = false, className = '', ...props }, ref) => {
    ...
    const wrapperDisplay = hasFlexClass ? 'flex' : (isBlockElement ? 'block' : 'inline-block');
    ...
    const wrapperClassName = `neo-shadow-wrapper ${disabled ? 'opacity-50' : ''} ${hasFlexClass ? 'h-full' : ''}`.trim();
    ...
    return (
      <span className={wrapperClassName} style={{ display: wrapperDisplay, position: 'relative' }}>
        <Component 
          ref={ref as any} 
